<?php
/***********************************************************************
*This PHP script is for Smart Thermostat
*Changes: Packet zero contains Total Image Size + Total Image CRC + 
*Packet CRC + footer
************************************************************************/
...
$hexheader = bin2hex($rawheader);							//write raw header in file
...
$logdata = "====\n#Time: ".date('r')							//server loads update data 
	...
	file_put_contents($my_log,$logdata,FILE_APPEND);			//writes update data into log image
...
file_put_contents($my_log,$packetcnt,FILE_APPEND);
$image_size = strlen(file_get_contents('./ComfortControl.bin',NULL,NULL,0)); //get image total size
if($packetcnt != 0)
{
	$newpacketcount = ($packetcnt - 1) * $Packet_Length;  				//start of a packet 
	$topacketcount =  $packetcnt * $Packet_Length;				//end of a packet
...
if( $packetcnt == 0 )						//send {Packetcount + Total image size + Total Image CRC + Packet CRC + *S* + ZGMP}
{
	$packetcnt_ascii = chr($packetcnt);				//ASCII packet count
	$dummy = file_get_contents('./ComfortControl.bin',NULL,NULL,0);		
	/* Generate total image size */
	$image_size_hex = dechex(strlen($dummy));
	$image_size_len = strlen($image_size_hex);
	PADZero($image_size_hex,$image_size_len);
    $image_size_ascii = pack('H*', $image_size_hex);            
	
	/* Generate total image CRC */
	$crc_hex = dechex(crc32gen($dummy,strlen($dummy)));			
	$my_len = strlen($crc_hex); 
	PADZero($crc_hex,$my_len);
	$CRC_Dummy_ascii = pack('H*',$crc_hex);						//Image CRC ASCII
		
	/* Generate Packet CRC */
	$PacketZeroData = $image_size_ascii.$CRC_Dummy_ascii;	    // combine data size and data CRC 
	$PacketZeroData_hex = dechex(crc32gen($PacketZeroData,8));	// calculate packet CRC
	$PacketZeroData_hex_len = strlen($PacketZeroData_hex); 
	PADZero($PacketZeroData_hex,$PacketZeroData_hex_len);
	$PacketZero_ascii = pack('H*',$PacketZeroData_hex);		    //Packet CRC ASCII 

	$outt = $packetcnt_ascii.$image_size_ascii.$CRC_Dummy_ascii.$PacketZero_ascii."*S*ZG";//1+4+4+4+5+2=20 bytes	
	
	$logdata = unpack('H*', $outt);			//ascii to hex
	file_put_contents($my_log,$logdata,FILE_APPEND);
	file_put_contents($my_log,$next_line,FILE_APPEND);
}
elseif(($newpacketcount < $image_size) && ($topacketcount < $image_size))
{	
	$logdata = "i m in\n";										    //for debug
	file_put_contents($my_log,$logdata,FILE_APPEND);	
	file_put_contents($my_log,$next_line,FILE_APPEND);
