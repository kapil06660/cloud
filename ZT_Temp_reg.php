...
$my_log = "ZT_Temp_outside_log.txt";
...
else 
{
	...
	{
		// check if it's a valid termostat SN
		$sql = "SELECT COUNT(*) FROM Product_Value_Objects WHERE Value_Owner_Product_SN='$thermSN'";
		$result = mysql_query($sql, $con);
		if ($result) {
			$row = mysql_fetch_row($result);
			file_put_contents($my_log, "No. of rows for Therm $thermSN: ". $row[0] . "\n\n", FILE_APPEND);
			if ($row[0] == 0) {
				// No rows exist for given thermSN
				// Return with invalid_thermsn status
				if ($first_out != 0) $packet_out .= "&";
				$packet_out .= "STATUS=invalid_thermsn";
				$first_out = 1;
		...
		// check if it's a valid remote SN
		$sql = "SELECT COUNT(*) FROM Product_Value_Objects WHERE Value_Owner_Product_SN='$remSN'";
		$result = mysql_query($sql, $con);
		if ($result) {
			$row = mysql_fetch_row($result);
			file_put_contents($my_log, "No. of rows for Remote $remSN: ". $row[0] . "\n\n", FILE_APPEND);
			if ($row[0] == 0) {
				// No rows exist for given remSN
				// Return with invalid_remotesn status
				if ($first_out != 0) $packet_out .= "&";
				$packet_out .= "STATUS=invalid_remotesn";
				$first_out = 1;
				sendResponse($packet_out);
				mysql_close($con);
				return;
