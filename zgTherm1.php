...
$rawwoheader = substr($rawstrip, 16);
...
$proheader = unpack('Vtimestamp/Vversion/Vserialno/vinterval/Cnumrec/Cflags',$rawheader); 
...
/* */  	
  	...
  	if($TempDataFound)
  	{
	  	reset($last_rec_array);
		while (list($key, $val) = each($last_rec_array)) 
		{
			if($key != 'type')
			{
				if($key == 'thermstatus')
